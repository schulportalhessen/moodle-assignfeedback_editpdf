...
class pdf extends TcpdfFpdi {
    ...
    private function get_gs_command_for_image(int $pageno, string $imagefile): string {
        ...
        $filename = \escapeshellarg($this->filename);
        $pagenoinc = \escapeshellarg($pageno + 1);
        // ToDo: Check if sDEVICE=png16m  is correct and annotations are visible in moodle.
    ...
    public static function ensure_pdf_file_compatible($tempsrc) {
        ...
        // First, quickest convertion option.
        if (!empty($CFG->pathtopdftoppm) && is_executable($CFG->pathtopdftoppm)) {
            // ToDo: We use ghostscript by default. Maybe there is a better way to use pdftoppm combine the files.
            $command = "$gsexec -q -sDEVICE=$outputdevice -dSAFER -dBATCH -dNOPAUSE -dPDFSETTINGS=/screen -sOutputFile=$tempdstarg $tempsrcarg";
        } else {
            // ToDo: Check if -dPDFSETTINGS=/screen should be used or not.
            // $command = "$gsexec -q -sDEVICE=$outputdevice         -dBATCH -dNOPAUSE -dPDFSETTINGS=/screen -sOutputFile=$tempdstarg $tempsrcarg"; // Moodle39
            // $command = "$gsexec -q -sDEVICE=$outputdevice -dSAFER -dBATCH -dNOPAUSE                       -sOutputFile=$tempdstarg $tempsrcarg"; // original in Moodle311
            $command = "$gsexec -q -sDEVICE=$outputdevice -dSAFER -dBATCH -dNOPAUSE -dPDFSETTINGS=/screen -sOutputFile=$tempdstarg $tempsrcarg";
        }
